...
function jacDavRitzHarm(trgBasis::Array{ComplexF64}, 
srcBasis::Array{ComplexF64}, kMat::Array{ComplexF64}, opt::Array{ComplexF64}, 
    vecDim::Integer, repDim::Integer, tol::Float64)::Tuple{Float64,Vector{Float64},Int32}
    ...
    errors = Vector{Float64}(undef, repDim)
    nb_it = 1 # Number of iterations
...
val,errors,nb_it = jacDavRitzHarm(trgBasis, srcBasis, kMat, opt, dims[1], dims[2], 1.0e-6)

number_of_iterations = LinRange(0, nb_it, nb_it+1)

print("nb_it ", nb_it, "\n")
# print("errors ", errors, "\n")

print("size(number_of_iterations) ", size(number_of_iterations), "\n")
print("size(errors) ", size(errors), "\n")

plot(number_of_iterations,errors)
xlabel!("Number of iterations")
ylabel!("Error")
savefig("Harmonic_Ritz_error_vs_nb_it.png")
