...
function davidson_it(A)
    ...
    for val = 1:1
        ...
        # for i = 2:cols # Old version 
            ...
            # New method 
            julia_eig_solve =  eigen(Hk_tilde[1:i,1:i])
            ...
            # New method 
            # Compute the smallest positive eigenpair (theta,s) of H_{k+1} 
            # with the norm(s) = 1
            # eigvals, eigenvectors = eigs(Hk_tilde, which=:SM)
            ...
            print("type(position) ", typeof(position), "\n")
            s = julia_eigvects[position][:]
...
print("Davidson smallest positive eigenvalue ",davidson_it(A), "\n")
